...
(function($) {
                ...
                f = {
                    ...
                    set: function(type, value) {
                        switch (type) {
                            case 'attrOpts':
                                ...
                                if (n.listInput === true) {
                                    n.listInput = $('<input type="hidden" name="' + s.attr('name').replace('[]', '').split('[').pop().replace(']', '') + '">').insertBefore(s)
                                }
                            ...
                            case 'nextInput':
                                ...
                                break;
                            case 'listInput':
                                if (n.listInput) n.listInput.val(value === null ? f.files.list(true) : value);
                                break
                        }
                    },
                    ...
                    thumbnails: {
                        ...
                        generateFileIcon: function(format, extension) {
                            var el = '<div style="${style}" class="fileuploader-item-icon' + '${class}"><i class="${name}"> ' + (extension ? extension : '') + '</i></div>';
                        ...
                        renderThumbnail: function(item, forceRender, src) {
                            var m = item.html.find('.fileuploader-item-image'),
                                source, setImageThumb = function(img) {
                                    var $img = $(img);
                                    if ($img.is('img')) $img.on('load error', function(e) {
                            ...
                            item.image = m;
                            if (['image', 'video', 'audio'].indexOf(item.format) > -1 && f.isFileReaderSupported() && (item.appended || n.thumbnails.startImageRenderer || forceRender)) {
                                ...
                                var load = function(src, fromReader) {
                                ...
                                if (src) load(src);
                                else if (typeof item.file == "string" && false) load(item.file);
                                else f.files.read(item, function() {
                        ...
                        popup: function(item) {
                            ...
                            if (p.find(n.thumbnails._selectors.popup).length) {
                                $.each(f._itFl, function(i, a) {
                                    if (a.popup && a.popup.close) {
                                        a.popup.html ? a.popup.html.remove() : null;
                                        a.popup.close()
                                    }
                                })
                            }
                            var template = $(f._assets.textParse(n.thumbnails.popup.template, item)),
                            ...
                            item.popup.close = function() {
                                ...
                                item.popup.html && n.thumbnails.popup.onHide && $.isFunction(n.thumbnails.popup.onHide) ? n.thumbnails.popup.onHide(item, l, p, o, s) : item.popup.html.remove();
                                $(window).off('keyup', windowEvent);
                                delete item.popup.html;
                                delete item.popup.close
                            };
                            if (item.reader.node) {
                                template.find('.fileuploader-popup-preview').html(item.reader.node);
                                item.reader.node.controls = true;
                                item.reader.node.currentTime = 0;
                                item.reader.node.play ? item.reader.node.play() : null
                            }
                            $(window).on('keyup', windowEvent);
                    ...
                    files: {
                        ...
                        list: function(toJson, customKey, triggered) {
                            ...
                            $.each(f._itFl, function(i, a) {
                                ...
                                var file = a;
                                if (toJson || customKey) {
                                    file = (file.choosed ? '' : '') + (customKey && f.files.getItemAttr(a, customKey) !== null ? f.files.getItemAttr(a, customKey) : a[typeof a.file == "string" ? "file" : "name"])
                                }
                        ...
                        remove: function(item, isFromCheck) {
                            ...
                            if (item.upload && item.upload.$ajax && item.upload.cancel) item.upload.cancel();
                            if (item.reader.src) {
                        ...
                        textParse: function(text, opts, noOptions) {
                            ...
                            switch (typeof(text)) {
                                case 'string':
                                    ...
                                    break;
                                case 'function':
                                    text = text(opts, l, p, o, s, f._assets.textParse);
                                    break
                            }
                            opts = null;
                            return text
    ...
    $.fn.fileuploader.defaults = {
        ...
        thumbnails: {
            ...
            popup: {
                template: '<div class="fileuploader-popup">' + '<div class="fileuploader-popup-preview">${reader.node}</div>' + '<div class="fileuploader-popup-tools">' + '<div class="left-side">' + '<p>Name:</p>' + '<h3>${name}</h3>' + '</div>' + '<div class="right-side">' + '<a class="fileuploader-popup-button fileuploader-popup-remove">${captions.remove}</a>' + '<a class="fileuploader-popup-button button-success fileuploader-popup-close">${captions.close}</a>' + '</div>' + '</div>' + '</div>',
                onShow: function(item) {
                    item.popup.html.on('click', '.fileuploader-popup-remove', function(e) {
                        item.popup.close();
                        item.remove()
                    }).on('click', '.fileuploader-popup-close', function(e) {
                        item.popup.close()
                    }).on('click', '.fileuploader-popup-preview', function(e) {
                        if (e.target === this) item.popup.close()
                    })
                },
