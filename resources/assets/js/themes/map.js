    ...
    features = current_form.find('.features-list input[type=checkbox]:checked').map(function(_, el) {
        ...
        if($("#houzez-listing-map").length > 0 || $('#mapViewHalfListings').length > 0 ) {
            ...
            if(current_tempalte == 'template/property-listings-map.php') {
                ...
                var state = VARIABLE_JS.search_state;
                var country = VARIABLE_JS.search_country;
                var keyword = VARIABLE_JS.search_keyword;
                var location = VARIABLE_JS.search_city;
                var features = VARIABLE_JS.search_feature;
                var area = VARIABLE_JS.search_area;
                var status = VARIABLE_JS.search_status;
                var type = VARIABLE_JS.search_type;
                var label = VARIABLE_JS.search_label;
                var property_id = VARIABLE_JS.property_id;
                var bedrooms = VARIABLE_JS.search_bedrooms;
                var bathrooms = VARIABLE_JS.search_bathrooms;
                var min_price = VARIABLE_JS.search_min_price;
                var max_price = VARIABLE_JS.search_max_price;
                var min_area = VARIABLE_JS.search_min_area;
                var max_area = VARIABLE_JS.search_max_area;
                var publish_date = VARIABLE_JS.search_publish_date;
                var search_no_posts = VARIABLE_JS.search_no_posts;

                var search_lat = VARIABLE_JS.search_lat;
                var search_long = VARIABLE_JS.search_long;
                var search_radius = VARIABLE_JS.search_radius;
                var search_location = VARIABLE_JS.search_location;
                var use_radius = VARIABLE_JS.use_radius;
